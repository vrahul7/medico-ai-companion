"""
Phase 1 Enhanced: Improved inference with more keywords and patterns.
Catches restaurants that were missed in the first pass.
"""
import pandas as pd
import re
import os
...
# Enhanced cuisine patterns - more comprehensive
CUISINE_PATTERNS = {
...
# Cuisine -> SKU map (same as before but with French added)
CUISINE_SKU_MAP = {
...
# Keyword boosters from business name
NAME_BOOSTERS = {
...
def main():
    print("=" * 60)
    print("PHASE 1 ENHANCED: IMPROVED INFERENCE ENGINE")
    print("=" * 60)
    ...
    # Apply known Swiggy IDs
    known_ids = {
        "Achayathis Restaurant": ("968291", "995338"),
        "Adyar Ananda Bhavan": ("13481", ""),
        "Copper Kitchen": ("13258", ""),
        "Buhari Hotel": ("50972", ""),
    }
    for idx2, row in result_df.iterrows():
        name = str(row["Restaurant"])
        for match_key, (fid, did) in known_ids.items():
            if match_key.lower() in name.lower():
                if fid: result_df.at[idx2, "Swiggy_Food_ID"] = fid
                if did: result_df.at[idx2, "Swiggy_Dineout_ID"] = did
                break
    
    # Save
    excel_path = os.path.join(OUTPUT_DIR, "SKU_Probability_Matrix_FINAL.xlsx")
    csv_path = os.path.join(OUTPUT_DIR, "SKU_Probability_Matrix_FINAL.csv")
