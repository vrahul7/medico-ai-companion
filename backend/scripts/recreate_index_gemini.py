"""
recreate_index_gemini.py — Reset Pinecone for Gemini Embeddings
==============================================================
Deletes the existing medico-ai-companion index and recreates it
at 768 dimensions to match Google text-embedding-004.

Run ONCE before Gemini ingestion:
    python scripts/recreate_index_gemini.py
"""
import os
import time
import sys
...
CORRECT_DIM      = 3072   # Verified gemini-embedding-001 dimension
METRIC           = "cosine"
CLOUD            = "aws"
REGION           = "us-east-1"
...
print("=" * 60)
print("  Medico AI -- Pinecone Index Reset (Gemini)")
print(f"  Target: {INDEX_NAME} @ {CORRECT_DIM} dims (text-embedding-004)")
print("=" * 60)
...
if INDEX_NAME in existing:
    ...
    if current_dim == CORRECT_DIM:
        print(f"[OK]   Dimension is already {CORRECT_DIM}. Will delete to clear any old vectors...")
        if not args.yes:
...
print(f"  Dimension:      {CORRECT_DIM}  (Google text-embedding-004)")
print(f"  Metric:         {METRIC}")
print(f"  Cloud/Region:   {CLOUD} / {REGION}")
print("=" * 60)
print()
print("[NEXT] Run ingestion:")
print("  python scripts/ingest_gemini.py --all")
print()
